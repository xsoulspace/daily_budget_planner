...
class _HomeScreenProvider extends StatelessWidget {
  const _HomeScreenProvider({required this.builder});
  final WidgetBuilder builder;
  @override
  Widget build(final BuildContext context) =>
      Portal(child: Builder(builder: builder));
}
        ...
        children: [
          ...
          const Expanded(
            child: Padding(
              ...
              child: TabBarView(children: [MonthlyView(), WeeklyView()]),
            ),
          ),
          const PredictionTeaserCard(),
          ConstrainedBox(
...
class UiTab extends StatelessWidget {
  const UiTab({required this.label, required this.iconData, super.key});
  final String label;
  ...
  Widget build(final BuildContext context) => Tab(
    ...
    child: Row(
      ...
      children: [Icon(iconData), const Gap(4), Text(label)],
    ),
  );
}
